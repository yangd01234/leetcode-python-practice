...
is present. Read in next the characters until the next non-digit charcter or the
end of the input is reached. The rest of the string is ignored.  Convert these 
digits into an integer (i.e. "123" -> 123, "0032" -> 32). If no digits were 
...
What test cases could you come across that you can simulate in your head before coding?
What can you use for constant time lookup?
How can you move items over and add them?
Why do you need to keep track of the length of the s?
How can yo ukeep track of the max and min int easily?
How can you use the function to your advantage?
Why should you use neg integer instead of true false for a negative flag?
"""
...
class Solution:
    def myAtoi(self, s: str) -> int:
        ...
        res = 0
        neg = 1
        # check for whitespace and keep moving forward until we see non-whitespace
        ...
        # check number 0-9 using set.  Breaks out to single chars.  Constant time
        check_num = set('0123456789')

        while i < len(s) and s[i] in check_num:
            res = res * 10 + int(s[i])
            i += 1

        res = res * neg

        # check max min constraint
        if res < 0:
...
sol = Solution()
print(sol.myAtoi("-testing1234123"))
print(sol.myAtoi("    testing1234123"))
print(sol.myAtoi("  tesg123412asdfasfd213431243214123412343124231423;:::::;3"))
print(sol.myAtoi("words and 987"))
print(sol.myAtoi("3.13159"))
